...
const GAME_HERO_TARGET_PRIORITY_DEFS: GDScript = preload("res://scripts/content/game_hero_target_priority_defs.gd")
...
const ARCHER_IMP_ARROW_TEXTURE: Texture2D = preload("res://assets/characters/packs/pack01/projectiles/arrow/Arrow01(100x100).png")
...
const HERO_COMBAT_TARGET_CHOICE_META: StringName = &"hero_combat_target_choice"
...
const BLIGHT_GAS_PULSE_DURATION: float = 0.52

static func door_wave_spawn_rng(game: Node, room_coord: Vector2i, spawn_order: int) -> RandomNumberGenerator:
	var spawn_rng: RandomNumberGenerator = RandomNumberGenerator.new()
	var seed_text: String = "%d:%d:%d:%d:%d:%d" % [
		int(game.network_simulation_seed),
		int(game.floor_index),
		int(game.wave_index),
		room_coord.x,
		room_coord.y,
		spawn_order,
	]
	spawn_rng.seed = int(hash(seed_text))
	return spawn_rng

static func crystal_pressure_spawn_rng(game: Node, event_index: int) -> RandomNumberGenerator:
	var spawn_rng: RandomNumberGenerator = RandomNumberGenerator.new()
	spawn_rng.seed = int(hash("%d:%d:%d" % [int(game.network_simulation_seed), int(game.floor_index), event_index]))
	return spawn_rng
...
static func clear_fighter_rage_throw_buff(game: Node, hero: Variant, announce_end: bool = false) -> bool:
	if hero == null or not is_instance_valid(hero):
		return false
	if String(hero.hero_class_id) != game.HERO_CLASS_FIGHTER:
		return false
	var had_buff: bool = int(hero.fighter_rage_throw_hits_left) > 0 or int(hero.fighter_rage_throw_level) > 0 or hero.has_meta(FIGHTER_RAGE_THROW_CONFIG_META)
	hero.fighter_rage_throw_hits_left = 0
	hero.fighter_rage_throw_level = 0
	if hero.has_meta(FIGHTER_RAGE_THROW_CONFIG_META):
		hero.remove_meta(FIGHTER_RAGE_THROW_CONFIG_META)
	if announce_end and had_buff:
...
static func consume_next_physical_card_modifiers(game: Node, hero: Variant, card_id: String) -> Dictionary:
	...
	if String(hero.hero_class_id) == game.HERO_CLASS_FIGHTER and card_id != "silver_gauntlet_toss_card":
		...
		if rage_value > 0:
			...
			var rage_knockback_bonus: float = float(rage_value) * PHYSICAL_CARD_RAGE_KNOCKBACK_FORCE_PER_LEVEL
			hero.fighter_rage = 0
			modifiers["rage_bonus_damage"] = rage_bonus
			modifiers["rage_bonus_knockback_force"] = rage_knockback_bonus
			game.add_resource_floating_text(
				hero.global_position + Vector2(0.0, -ROGUE_COMBO_POPUP_Y_OFFSET),
				"Rage +%d / KB %d" % [int(rage_bonus), rage_value],
				FIGHTER_RAGE_POPUP_COLOR
			)
	return modifiers
...
static func apply_thrash_around_hit_effect(game: Node, hero: Variant, enemy: Variant, impact_direction: Vector2) -> void:
	...
	if hits_left <= 0:
		...
		return
	var rage_level: int = clampi(int(hero.fighter_rage_throw_level), 0, maxi(int(hero.fighter_rage_max), 0))
	if rage_level <= 0:
		clear_fighter_rage_throw_buff(game, hero)
		return
	var throw_config: Dictionary = Dictionary(hero.get_meta(FIGHTER_RAGE_THROW_CONFIG_META, {})).duplicate(true)
	if throw_config.is_empty():
		clear_fighter_rage_throw_buff(game, hero)
		return
	...
	if can_apply_throw and target_room != game.INVALID_ROOM and game.rooms.has(target_room):
		var knockback_force: float = maxf(float(throw_config.get("knockback_force", 0.0)), 0.0)
		var knockback_duration: float = clampf(float(throw_config.get("knockback_duration", 0.16)), 0.04, 0.65)
		var allowed_bounces: int = maxi(0, int(throw_config.get("max_wall_bounces", 0)))
		var flatfooted_duration: float = maxf(float(throw_config.get("flatfooted_duration", 0.0)), 0.0)
		var flatfooted_move_multiplier: float = clampf(float(throw_config.get("flatfooted_move_multiplier", 1.0)), 0.0, 1.0)
		var flatfooted_attack_speed_multiplier: float = clampf(float(throw_config.get("flatfooted_attack_speed_multiplier", 1.0)), 0.0, 1.0)
		var flatfooted_damage_taken_multiplier: float = maxf(float(throw_config.get("flatfooted_damage_taken_multiplier", 1.0)), 1.0)
		game.knockback_actor(
			...
			target_room,
			{
				...
				"bounce_effect_color": Color(throw_config.get("bounce_effect_color", Color.WHITE)),
			}
		)
...
static func apply_hero_on_enemy_hit_effects(game: Node, hero: Variant, enemy: Variant, impact_direction: Vector2 = Vector2.RIGHT) -> void:
	if hero == null or not is_instance_valid(hero) or enemy == null or not is_instance_valid(enemy):
		return
	...
	apply_thrash_around_hit_effect(game, hero, enemy, resolved_impact_direction)
	if not hero_is_combo_class(game, hero):
		return
...
static func launch_wave(game: Node, entered_room: Vector2i) -> void:
	...
	var wave_rng: RandomNumberGenerator = door_wave_spawn_rng(game, entered_room, -1)
	if dark_rooms.has(entered_room):
		...
		while chosen_rooms.size() < 1 + bonus_spawn_room_count and not dark_rooms.is_empty():
			var picked_room: Vector2i = pick_spawn_room(game, dark_rooms, priority_dark_rooms, wave_rng)
...
static func advance_single_wave_spawn_build(game: Node, pending_build: Dictionary) -> bool:
	...
	if phase == "plan":
		var spawn_rng: RandomNumberGenerator = door_wave_spawn_rng(game, room_coord, int(pending_build.get("spawn_order", 0)))
		var spawn_plan: Array[String] = build_enemy_spawn_plan(game, maxi(int(pending_build.get("wave_points", 0)), 1), false, spawn_rng)
		if spawn_plan.is_empty():
			return false
		pending_build["spawn_plan"] = spawn_plan
		pending_build["positions"] = []
		pending_build["cluster_anchor"] = centered_spawn_anchor(game, room_coord, spawn_rng)
		pending_build["cluster_radius"] = spawn_cluster_radius_for_room(game, room_coord, spawn_plan.size())
		pending_build["cluster_base_angle"] = spawn_rng.randf() * TAU
...
static func trigger_crystal_pressure(game: Node) -> void:
	...
	for room_coord in dark_rooms:
		if bool(game.rooms[room_coord].get("feature_spawn_priority", false)):
			priority_dark_rooms.append(room_coord)
	var pressure_rng: RandomNumberGenerator = crystal_pressure_spawn_rng(game, game.crystal_pressure_event_index)
	game.crystal_pressure_event_index += 1
	var chosen_room: Vector2i = pick_spawn_room(game, dark_rooms, priority_dark_rooms, pressure_rng)
	if chosen_room == game.INVALID_ROOM:
		chosen_room = dark_rooms[pressure_rng.randi_range(0, dark_rooms.size() - 1)]
	var pressure_spawn_points: int = game.CRYSTAL_PRESSURE_ENEMIES_PER_ROOM
	var queued_count: int = queue_pressure_spawn(game, chosen_room, pressure_spawn_points, remaining_capacity, pressure_rng)
	game.crystal_pressure_timer_left = crystal_pressure_interval_for_dark_room_count(game, dark_rooms.size())
	if queued_count <= 0:
		return
...
static func pick_spawn_room(game: Node, dark_rooms: Array[Vector2i], priority_dark_rooms: Array[Vector2i], random_source: RandomNumberGenerator = null) -> Vector2i:
	if dark_rooms.is_empty():
		return game.INVALID_ROOM
	var spawn_rng: RandomNumberGenerator = random_source if random_source != null else game.rng
	if not priority_dark_rooms.is_empty() and spawn_rng.randf() < 0.7:
		return priority_dark_rooms[spawn_rng.randi_range(0, priority_dark_rooms.size() - 1)]
	return dark_rooms[spawn_rng.randi_range(0, dark_rooms.size() - 1)]

static func queue_pressure_spawn(game: Node, room_coord: Vector2i, count: int, max_spawn_entities: int = -1, random_source: RandomNumberGenerator = null) -> int:
	if not game.rooms.has(room_coord) or count <= 0:
		return 0
	var spawn_rng: RandomNumberGenerator = random_source if random_source != null else game.rng
	var spawn_plan: Array[String] = build_enemy_spawn_plan(game, count, true, spawn_rng)
	if max_spawn_entities >= 0 and spawn_plan.size() > max_spawn_entities:
		spawn_plan.resize(max_spawn_entities)
	if spawn_plan.is_empty():
		return 0
	var cluster_anchor: Vector2 = centered_spawn_anchor(game, room_coord, spawn_rng)
	var cluster_radius: float = spawn_cluster_radius_for_room(game, room_coord, spawn_plan.size())
	var cluster_base_angle: float = spawn_rng.randf() * TAU
	var clustered_positions: Array = build_spawn_positions(game, room_coord, spawn_plan.size(), cluster_anchor, cluster_radius, cluster_base_angle)
	var marker_lead: float = maxf(float(game.WAVE_PRESPAWN_MARKER_LEAD), 0.0)
...
static func weighted_enemy_type_choice(game: Node, candidates: Array[String], pressure_spawn: bool = false, random_source: RandomNumberGenerator = null) -> String:
	...
	var spawn_rng: RandomNumberGenerator = random_source if random_source != null else game.rng
	var roll: float = spawn_rng.randf() * maxf(total_weight, 0.001)
	for enemy_type in candidates:
...
static func build_enemy_spawn_plan(game: Node, budget: int, pressure_spawn: bool = false, random_source: RandomNumberGenerator = null) -> Array[String]:
	...
	while remaining > 0:
		...
		var chosen_type: String = weighted_enemy_type_choice(game, candidates, pressure_spawn, random_source)
...
static func centered_spawn_anchor(game: Node, room_coord: Vector2i, random_source: RandomNumberGenerator = null) -> Vector2:
	...
	var spawn_rng: RandomNumberGenerator = random_source if random_source != null else game.rng
	var jitter_angle: float = spawn_rng.randf() * TAU
	var jitter_distance: float = spawn_rng.randf() * SPAWN_CENTER_ANCHOR_JITTER
...
static func spawn_wave_enemy_at(game: Node, room_coord: Vector2i, enemy_type: String, spawn_position_hint: Vector2 = Vector2.INF, spawn_source: String = "door_wave") -> Variant:
...
static func apply_card_projectile_hits(game: Node, projectile: Dictionary) -> void:
	...
	for enemy in game.enemies:
		...
		if owner_hero != null and is_instance_valid(owner_hero):
			apply_hero_on_enemy_hit_effects(game, owner_hero, enemy, impact_direction)
...
static func apply_party_passive_auras(game: Node, delta: float) -> void:
	const AURA_RADIUS: float = 132.0
	const CLERIC_RESTORATION_PER_SECOND: float = 3.0
	const FIGHTER_LEADERSHIP_DAMAGE_MULTIPLIER: float = 1.15
	for hero_variant in game.heroes:
		var hero: Variant = hero_variant
		if hero.has_method("clear_leadership_aura"):
			hero.clear_leadership_aura()
	for cleric_variant in game.heroes:
		var cleric: Variant = cleric_variant
		if not game.hero_is_active(cleric) or not game.hero_has_cleric_restoration_aura(cleric):
			continue
		if cleric.pending_room != game.HERO_INVALID_ROOM:
			continue
		for ally_variant in game.heroes:
			var ally: Variant = ally_variant
			if not game.hero_is_active(ally) or Vector2i(ally.current_room) != Vector2i(cleric.current_room):
				continue
			if ally.global_position.distance_to(cleric.global_position) <= AURA_RADIUS:
				ally.heal(CLERIC_RESTORATION_PER_SECOND * delta)
	for fighter_variant in game.heroes:
		var fighter: Variant = fighter_variant
		if not game.hero_is_active(fighter) or not game.hero_has_fighter_leadership_aura(fighter):
			continue
		if fighter.pending_room != game.HERO_INVALID_ROOM:
			continue
		for ally_variant in game.heroes:
			var ally: Variant = ally_variant
			if ally == fighter or not game.hero_is_active(ally) or Vector2i(ally.current_room) != Vector2i(fighter.current_room):
				continue
			if ally.global_position.distance_to(fighter.global_position) <= AURA_RADIUS and ally.has_method("apply_leadership_aura"):
				ally.apply_leadership_aura(FIGHTER_LEADERSHIP_DAMAGE_MULTIPLIER)

static func process_combat(game: Node, delta: float) -> void:
	advance_room_sanctuary_effects(game, delta)
	apply_party_passive_auras(game, delta)
	...
	for hero in game.heroes:
		if not game.hero_is_active(hero):
			continue
		var basic_attack_id: String = game.hero_basic_attack_id(hero)
		var basic_attack_is_melee: bool = game.attack_delivery(basic_attack_id) == "melee"
		...
		if hero.has_method("has_active_scorcher_channel") and bool(hero.has_active_scorcher_channel()):
			continue
		if basic_attack_is_melee and not hero.carrying_crystal and hero.pending_room == game.HERO_INVALID_ROOM and not game.hero_has_locked_player_command(hero) and game.active_hand_drag.is_empty():
			var pursuit_target: Variant = hero_combat_target(game, hero)
		...
		if hero.carrying_crystal or hero.pending_room != game.HERO_INVALID_ROOM or hero.cooldown_left > 0.0 or game.attacker_has_pending_melee(hero):
			continue
		if basic_attack_is_melee:
			var melee_target: Variant = hero_combat_target(game, hero)
			...
			if not execute_attack(game, hero, melee_target, basic_attack_id):
				continue
			note_hero_combo_attack(game, hero)
			hero.cooldown_left = hero.current_attack_cooldown()
			continue
		var hero_target: Variant = hero_combat_target(game, hero)
		if hero_target != null:
			var target_offset: Vector2 = hero_target.global_position - hero.global_position
			var target_distance: float = target_offset.length()
			if target_distance > hero.attack_range:
				if game.active_hand_drag.is_empty() and not game.hero_has_locked_player_command(hero):
					var approach_direction: Vector2 = target_offset.normalized() if target_distance > 0.001 else Vector2.RIGHT
					var approach_position: Vector2 = game.clamp_point_to_room(hero_target.global_position - approach_direction * maxf(hero.attack_range - 10.0, 28.0), hero.current_room)
					var current_room_path: Array[Vector2i] = [hero.current_room]
					game.issue_hero_steps(hero, game.build_steps_for_path(current_room_path, hero.global_position, approach_position))
				continue
			if not execute_attack(game, hero, hero_target, basic_attack_id):
				continue
			note_hero_combo_attack(game, hero)
			hero.cooldown_left = hero.current_attack_cooldown()
	for hero in game.heroes:
		...
		if game.hero_has_locked_player_command(hero) or not game.active_hand_drag.is_empty():
			continue
		if hero_combat_target(game, hero) != null:
			continue
...
static func process_modules(game: Node, delta: float) -> void:
	game.reconcile_module_actors()
	var cleric_menders_by_room: Dictionary = collect_cleric_menders_by_room(game)
	var targetable_enemies_by_room: Dictionary = GAME_TARGETING_FLOW.targetable_hostile_enemies_by_room(game)
	for room_coord_variant in game.rooms.keys():
		var room_coord: Vector2i = room_coord_variant
		var room: Dictionary = game.rooms[room_coord]
		...
		var blight_pulse_was_active: bool = float(room.get("blight_pulse_time_left", 0.0)) > 0.0
		var blight_pulse_time_left: float = maxf(float(room.get("blight_pulse_time_left", 0.0)) - delta, 0.0)
		var blight_pulse_progress: float = 1.0 - blight_pulse_time_left / BLIGHT_GAS_PULSE_DURATION
		room["blight_pulse_time_left"] = blight_pulse_time_left
		room["blight_pulse_strength"] = sin(PI * clampf(blight_pulse_progress, 0.0, 1.0)) if blight_pulse_time_left > 0.0 else 0.0
		...
		for module_index in range(room["minor_modules"].size()):
			...
			var module_targeting: Dictionary = game.minor_module_targeting_definition(module_type)
			var module_targets: Array = GAME_TARGETING_FLOW.select_actor_targets(slot_position, room_targetable_enemies, String(module_targeting.get("strategy", "nearest")), float(module_targeting.get("range", 0.0)))
			match module_type:
				game.MINOR_MODULE_BLIGHT_GAS:
					...
					for enemy_variant in module_targets:
						var enemy: Variant = enemy_variant
						if not game.enemy_is_active(enemy):
							continue
					...
					if gas_hit:
						...
						room["blight_pulse_time_left"] = BLIGHT_GAS_PULSE_DURATION
						module_data["cooldown"] = game.minor_module_cooldown(module_type)
				game.MINOR_MODULE_RUNEBURST_MORTAR:
					var mortar_target: Variant = module_targets[0] if not module_targets.is_empty() else null
					if mortar_target != null:
						module_data["cooldown"] = game.minor_module_cooldown(module_type)
						var splash_center: Vector2 = mortar_target.global_position
						spawn_module_visual_projectile(game, "mortar_shell", slot_position, splash_center, game.minor_module_color(module_type), 4.6, 780.0)
				...
				game.MINOR_MODULE_ARCANA_TURRET:
					var arcana_target: Variant = module_targets[0] if not module_targets.is_empty() else null
					if arcana_target != null:
						var arcana_level: int = clampi(game.minor_module_level(module_type), 1, 4)
						var arcana_damage_cap: float = float(game.minor_module_arcana_turret_max_damage(arcana_level))
						var arcana_damage: float = minf(maxf(float(game.science), 0.0), arcana_damage_cap)
						module_data["cooldown"] = game.minor_module_cooldown(module_type)
						spawn_projectile(game, slot_position, arcana_target, arcana_damage, game.minor_module_attack_definition(module_type), "Arcana Turret")
				game.MINOR_MODULE_CONVERSION:
					var conversion_target: Variant = module_targets[0] if not module_targets.is_empty() else null
					if conversion_target != null:
						...
						spawn_module_visual_projectile(game, "soulbind_orb", slot_position, conversion_center, game.minor_module_color(module_type), 4.2, 860.0)
				...
				game.MINOR_MODULE_BOUNTY_INDUSTRY, game.MINOR_MODULE_BOUNTY_FOOD, game.MINOR_MODULE_BOUNTY_SCIENCE:
					pass
				_:
					var turret_target: Variant = module_targets[0] if not module_targets.is_empty() else null
					if turret_target == null:
						room["minor_modules"][module_index] = module_data
						continue
					module_data["cooldown"] = game.minor_module_cooldown(module_type)
					spawn_projectile(game, slot_position, turret_target, game.minor_module_damage(module_type), game.minor_module_attack_definition(module_type), "Crossbow")
			room["minor_modules"][module_index] = module_data
		game.rooms[room_coord] = room
		if blight_pulse_was_active or float(room.get("blight_pulse_time_left", 0.0)) > 0.0:
			game.sync_static_room_instance(room_coord)
...
static func attack_definition(game: Node, attack_id: String) -> Dictionary:
	var definition: Dictionary = GAME_HERO_DEFS.attack_definition(attack_id)
	if definition.is_empty():
		definition = GAME_ENEMY_DEFS.attack_definition(attack_id)
	if definition.is_empty():
		definition = GAME_CARD_DEFS.attack_definition(game, attack_id)
	return definition

static func attack_delivery(game: Node, attack_id: String) -> String:
	return String(attack_definition(game, attack_id).get("delivery", ""))

static func execute_attack(game: Node, attacker: Variant, target: Variant, attack_id: String) -> bool:
	if attacker == null or target == null or not is_instance_valid(attacker) or not is_instance_valid(target):
		return false
	var definition: Dictionary = attack_definition(game, attack_id)
	...
	attacker.trigger_attack(target_position, String(definition.get("visual_style", "")))
	match String(definition.get("delivery", "melee")):
		"fireball":
			if game.is_module_actor(target):
				target.take_damage(damage, Vector2.RIGHT, source_label)
			var defeated_heroes: Array[String] = game.explode_enemy_fireball(attack_room, target_position, damage, float(definition.get("blast_radius", 68.0)), float(definition.get("blast_force", 360.0)), source_label)
			if defeated_heroes.size() == 1:
				game.status_message = String(enemy_definition.get("attack_single_defeat_status_template", "%s defeated %s.")) % [source_label, defeated_heroes[0]]
			elif defeated_heroes.size() > 1:
				game.status_message = String(enemy_definition.get("attack_multiple_defeat_status", "%s defeated multiple heroes.")) % source_label
			else:
				game.status_message = String(enemy_definition.get("attack_status_template", "%s attacks %s.")) % [source_label, attack_target_label(game, target)]
		"melee":
			game.queue_pending_melee_attack(attacker, target, damage, attacker.melee_impact_delay(), source_label)
			if game.is_enemy_actor(attacker):
				game.status_message = String(enemy_definition.get("attack_status_template", "%s attacks %s.")) % [source_label, attack_target_label(game, target)]
		_:
			spawn_projectile(game, attacker.global_position, target, damage, definition, source_label)
	return true

static func attack_target_label(game: Node, target: Variant) -> String:
	if game.is_hero_actor(target):
		return String(target.hero_name)
	if game.is_enemy_actor(target):
		return String(target.get_meta("summon_source_label", "a summoned ally")).to_lower()
	if game.is_module_actor(target):
		return game.build_type_label(String(target.module_type)).to_lower()
	return "a target"

static func apply_projectile_curve(projectile: Dictionary, origin: Vector2, target_position: Vector2, speed: float, curve_offset: float) -> void:
	if absf(curve_offset) <= 0.001:
		return
	var initial_direction: Vector2 = (target_position - origin).normalized()
	if initial_direction == Vector2.ZERO:
		initial_direction = Vector2.RIGHT
	projectile["velocity"] = initial_direction.rotated(curve_offset) * maxf(speed, 1.0)
	projectile["curve_turn_rate"] = 5.5

static func spawn_projectile(game: Node, origin: Vector2, target: Variant, damage: float, definition: Dictionary, source_label: String = "") -> void:
	var target_position: Vector2 = target.global_position if is_instance_valid(target) else origin
	var delivery: String = String(definition.get("delivery", "laser"))
	var projectile: Dictionary = {
		"kind": String(definition.get("projectile_kind", delivery)),
		"motion": String(definition.get("projectile_motion", "direct")),
		"position": origin,
		"previous": origin,
		"target": target,
		"target_position": target_position,
		"damage": float(definition.get("damage", definition.get("base_damage", damage))),
		"speed": float(definition.get("projectile_speed", 950.0)),
		"color": definition.get("projectile_color", definition.get("color", Color.WHITE)),
		"width": float(definition.get("projectile_width", 4.0)),
		"source_label": source_label,
		"expose_stacks": int(definition.get("expose_stacks_per_hit", 0)),
		"expose_duration": float(definition.get("expose_duration", 0.0)),
		"knockback_force": float(definition.get("knockback_force", 0.0)),
		"knockback_duration": float(definition.get("knockback_duration", 0.16)),
	}
	if String(projectile["motion"]) == "homing":
		...
		if String(definition.get("projectile_initial_direction", "")) == "perpendicular":
			var curve_sign: float = 1.0 if float(definition.get("projectile_curve_offset", 0.0)) >= 0.0 else -1.0
			launch_direction = Vector2(-target_direction.y, target_direction.x) * curve_sign
		projectile.merge({
			"velocity": Vector2.ZERO,
			"travel_direction": launch_direction,
			"current_speed": 0.0,
			"acceleration": float(definition.get("projectile_acceleration", 2400.0)),
			"curve_turn_rate": float(definition.get("projectile_turn_rate", 3.0)),
			"glow_radius": float(definition.get("projectile_glow_radius", 14.0)),
			"hit_radius": float(definition.get("projectile_hit_radius", 12.0)),
		})
	else:
		apply_projectile_curve(projectile, origin, target_position, float(projectile["speed"]), float(definition.get("projectile_curve_offset", 0.0)))
	game.projectiles.append(projectile)

static func spawn_module_visual_projectile(game: Node, projectile_kind: String, origin: Vector2, target_position: Vector2, color: Color, width: float, speed: float) -> void:
	if origin.distance_to(target_position) <= 6.0:
		return
	game.projectiles.append({
...
static func apply_projectile_hit(game: Node, projectile: Dictionary, target: Variant, impact_direction: Vector2) -> void:
	if target == null or not is_instance_valid(target):
		return
	var resolved_direction: Vector2 = impact_direction.normalized()
	if resolved_direction == Vector2.ZERO:
		resolved_direction = Vector2.RIGHT
	var damage: float = float(projectile.get("damage", 0.0))
	if game.is_enemy_actor(target) and game.enemy_is_active(target):
		target.take_damage(damage, resolved_direction)
		var knockback_force: float = projectile_total_knockback_force(projectile)
		if knockback_force > 0.0:
			game.knockback_actor(target, resolved_direction, knockback_force, clampf(float(projectile.get("knockback_duration", 0.16)), 0.08, 0.5), target.current_room)
		return
	if game.is_module_actor(target) and target.is_active():
		target.take_damage(damage, resolved_direction, String(projectile.get("source_label", "A ranged attack")))
		return
	if not game.is_hero_actor(target) or not game.hero_is_active(target):
		return
	var was_invulnerable: bool = float(target.invulnerability_time_left) > 0.0
	var adjusted_damage: float = game.adjusted_incoming_damage_for_hero(target, damage)
	var previous_rage: int = int(target.fighter_rage)
	var defeated: bool = target.take_damage(adjusted_damage, false)
	if not was_invulnerable and int(projectile.get("expose_stacks", 0)) > 0 and target.has_method("apply_expose_stack"):
		target.apply_expose_stack(int(projectile["expose_stacks"]), 3, float(projectile.get("expose_duration", 0.0)))
	maybe_show_fighter_rage_popup(game, target, previous_rage)
	if defeated and game.try_auto_cast_fatal_shield(target, adjusted_damage):
		return
	if defeated:
...
static func advance_projectiles(game: Node, delta: float) -> void:
	...
	for projectile in game.projectiles:
		...
		if bool(projectile.get("network_visual_only", false)):
			var visual_position: Vector2 = Vector2(projectile.get("position", Vector2.ZERO))
			var visual_target: Vector2 = Vector2(projectile.get("target_position", visual_position))
			var visual_offset: Vector2 = visual_target - visual_position
			if visual_offset.length() <= 6.0:
				continue
			projectile["previous"] = visual_position
			projectile["position"] = visual_position.move_toward(visual_target, float(projectile.get("speed", game.PROJECTILE_SPEED)) * delta)
		...
		if projectile_kind == "fireball_blast" or projectile_kind == "shield_flash" or projectile_kind == "calm_emotions_wave" or projectile_kind == "lightning_bolt" or projectile_kind == "gas_pulse" or projectile_kind == "necromancer_attack_effect" or projectile_kind == "priest_heal_effect" or projectile_kind == "priest_attack_effect" or projectile_kind == "web_field" or projectile_kind == "ghostfire_ray" or projectile_kind == "scorcher_flame_cone" or projectile_kind == "dust_burst":
			...
			active_projectiles.append(projectile)
			continue
		if String(projectile.get("motion", "direct")) == "homing":
			...
			if missile_offset.length() <= missile_hit_radius:
				apply_projectile_hit(game, projectile, missile_target, missile_offset if missile_offset.length() > 0.001 else missile_travel_direction)
				continue
			...
			if missile_offset.length() <= close_assist_distance:
				...
				if assisted_position.distance_to(missile_target_position) <= missile_hit_radius or game.point_distance_to_segment(missile_target_position, missile_position, assisted_position) <= missile_hit_radius:
					apply_projectile_hit(game, projectile, missile_target, desired_direction)
					continue
				active_projectiles.append(projectile)
				continue
			...
			if missile_next_position.distance_to(missile_target_position) <= missile_hit_radius or game.point_distance_to_segment(missile_target_position, missile_position, missile_next_position) <= missile_hit_radius:
				apply_projectile_hit(game, projectile, missile_target, missile_velocity if missile_velocity.length() > 0.001 else desired_direction)
				continue
		...
		if offset.length() <= 6.0:
			var impact_direction_simple: Vector2 = offset if offset.length() > 0.001 else current_position_simple - previous_position_simple
			if impact_direction_simple.length() <= 0.001:
				impact_direction_simple = Vector2(projectile.get("velocity", Vector2.RIGHT))
			apply_projectile_hit(game, projectile, target, impact_direction_simple)
			continue
		var travel_direction: Vector2 = offset.normalized()
		if projectile.has("curve_turn_rate"):
			var projectile_speed: float = maxf(float(projectile["speed"]), 1.0)
			var projectile_velocity: Vector2 = Vector2(projectile.get("velocity", travel_direction * projectile_speed))
			projectile_velocity = projectile_velocity.move_toward(travel_direction * projectile_speed, projectile_speed * float(projectile["curve_turn_rate"]) * delta)
			if projectile_velocity.length() > 0.001:
				travel_direction = projectile_velocity.normalized()
		...
		projectile["position"] = current_position_simple + travel_direction * travel_distance
		active_projectiles.append(projectile)
...
static func draw_projectiles(game: Node, canvas: CanvasItem = null) -> void:
	...
	for projectile in game.projectiles:
		...
		if projectile_kind == "axe":
			...
			surface.draw_line(previous, current_position, Color(color.r, color.g * 0.72, color.b * 0.40, 0.52), width + 2.0, true)
			surface.draw_circle(current_position, radius * 0.55, color)
			surface.draw_line(current_position + Vector2.RIGHT.rotated(angle) * radius, current_position - Vector2.RIGHT.rotated(angle) * radius, color.lightened(0.24), 5.0, true)
			surface.draw_line(current_position + Vector2.UP.rotated(angle) * (radius * 0.8), current_position - Vector2.UP.rotated(angle) * (radius * 0.8), color.lightened(0.55), 3.0, true)
			continue
		if projectile_kind == "dagger":
			...
			var is_shadow_dagger: bool = String(projectile.get("card_id", "")) == "rogue_combo_dagger_card"
			if is_shadow_dagger:
				surface.draw_line(previous, current_position, Color(color.r, color.g * 0.55, color.b, 0.58), width + 2.4, true)
			surface.draw_colored_polygon(PackedVector2Array([tip, tail_left, tail_right]), color)
			var blade_highlight: Color = Color("54f0e1") if is_shadow_dagger else Color("eff8ff")
			surface.draw_line(current_position - direction * 10.0, current_position + direction * 4.0, blade_highlight, 2.0, true)
			continue
		if projectile_kind == "arrow":
			...
			surface.draw_set_transform(current_position, arrow_direction.angle(), Vector2.ONE)
			surface.draw_texture_rect(ARCHER_IMP_ARROW_TEXTURE, Rect2(-36.0, -36.0, 72.0, 72.0), false, Color(color.r, color.g, color.b, 1.0), false)
		...
		if projectile_kind == "magic_missile":
			...
			continue
		if projectile_kind == "gas_glob" or projectile_kind == "mortar_shell" or projectile_kind == "arcana_bolt" or projectile_kind == "soulbind_orb":
			var module_delta: Vector2 = current_position - previous
			var module_direction: Vector2 = module_delta.normalized()
			if module_direction == Vector2.ZERO:
				module_direction = (Vector2(projectile.get("target_position", current_position)) - current_position).normalized()
			if module_direction == Vector2.ZERO:
				module_direction = Vector2.RIGHT
			var glow_radius: float = maxf(width * 3.8, 13.0)
			var trail_color: Color = Color(color.r, color.g, color.b, 0.44)
			var core_color: Color = Color(0.92, 1.0, 0.96, 0.94)
			if projectile_kind == "mortar_shell":
				glow_radius = maxf(width * 3.4, 15.0)
				core_color = Color("fff0be")
				trail_color = Color(1.0, color.g * 0.64, color.b * 0.36, 0.56)
			elif projectile_kind == "arcana_bolt":
				core_color = Color("e9f3ff")
			elif projectile_kind == "soulbind_orb":
				core_color = Color("d5fff0")
			surface.draw_line(previous, current_position, Color(trail_color.r, trail_color.g, trail_color.b, 0.24), width + 10.0, true)
			surface.draw_line(previous, current_position, trail_color, width + 4.0, true)
			surface.draw_line(previous, current_position, core_color, maxf(width * 0.5, 1.8), true)
			surface.draw_circle(current_position, glow_radius, Color(color.r, color.g, color.b, 0.14))
			surface.draw_circle(current_position, glow_radius * 0.58, Color(color.r, color.g, color.b, 0.34))
			var module_time: float = float(Time.get_ticks_msec()) / 1000.0
			if projectile_kind == "mortar_shell":
				surface.draw_circle(current_position - module_direction * 2.0, maxf(width * 1.35, 5.0), Color("502518"))
				surface.draw_circle(current_position + module_direction * 1.4, maxf(width * 0.88, 3.4), core_color)
				for spark_index in range(3):
					var spark_angle: float = module_direction.angle() + PI + float(spark_index - 1) * 0.48 + sin(module_time * 9.0 + float(spark_index)) * 0.18
					var spark_position: Vector2 = current_position + Vector2.RIGHT.rotated(spark_angle) * (8.0 + float(spark_index) * 3.0)
					surface.draw_circle(spark_position, 1.8, Color("ffd780"))
			elif projectile_kind == "gas_glob":
				for bubble_index in range(3):
					var bubble_angle: float = module_time * 4.2 + float(bubble_index) * TAU / 3.0
					var bubble_position: Vector2 = current_position + Vector2.RIGHT.rotated(bubble_angle) * (4.0 + float(bubble_index) * 1.8)
					surface.draw_circle(bubble_position, 2.2 + float(bubble_index) * 0.45, Color(0.86, 1.0, 0.72, 0.82))
			elif projectile_kind == "arcana_bolt":
				var crystal_perpendicular: Vector2 = Vector2(-module_direction.y, module_direction.x)
				var crystal_tip: Vector2 = current_position + module_direction * 8.0
				var crystal_tail: Vector2 = current_position - module_direction * 6.0
				surface.draw_colored_polygon(PackedVector2Array([crystal_tip, current_position + crystal_perpendicular * 5.0, crystal_tail, current_position - crystal_perpendicular * 5.0]), core_color)
				var rune_angle: float = module_time * 7.0
				surface.draw_arc(current_position, 10.0, rune_angle, rune_angle + PI * 1.45, 12, Color(color.r, color.g, color.b, 0.86), 1.5, true)
			elif projectile_kind == "soulbind_orb":
				for wisp_index in range(2):
					var wisp_angle: float = module_time * 8.0 + float(wisp_index) * PI
					var wisp_position: Vector2 = current_position + Vector2.RIGHT.rotated(wisp_angle) * 8.0
					surface.draw_circle(wisp_position, 2.5, Color("b98cff"))
			else:
				surface.draw_circle(current_position + module_direction * 1.6, maxf(width * 0.98, 3.3), core_color)
			continue
		...
		if projectile_kind == "priest_attack_effect":
			...
			continue
		if projectile_kind == "imp_arrow":
			var arrow_direction: Vector2 = current_position - previous
			if arrow_direction.length() <= 0.001:
				arrow_direction = (current_position - Vector2(projectile.get("target_position", current_position))).normalized()
			surface.draw_set_transform(current_position, arrow_direction.angle(), Vector2.ONE)
			surface.draw_texture_rect(ARCHER_IMP_ARROW_TEXTURE, Rect2(-16.0, -16.0, 32.0, 32.0), false, Color(color.r, color.g, color.b, 1.0), false)
			surface.draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE)
			continue
...
static func hero_combat_target(game: Node, hero: Variant) -> Variant:
	if hero == null or not is_instance_valid(hero):
		return null
	var priority_table: Dictionary = GAME_HERO_TARGET_PRIORITY_DEFS.priority_table_for_hero(hero)
	return GAME_TARGETING_FLOW.select_in_room_actor_target(
		game,
		hero,
		GAME_TARGETING_FLOW.targetable_hostile_enemies_in_room(game, Vector2i(hero.current_room)),
		priority_table,
		HERO_COMBAT_TARGET_CHOICE_META
	)
...
static func cleanup_enemies(game: Node) -> void:
	...
	for enemy in game.enemies:
		if not is_instance_valid(enemy):
			continue
		if bool(enemy.get_meta("slime_split_pending", false)):
			var split_health: float = maxf(float(enemy.get_meta("slime_split_health", 0.0)), 0.0)
			if String(enemy.enemy_role) == game.ENEMY_TYPE_SLIME and split_health > 0.0:
				var spawn_source: String = String(enemy.get_meta("spawn_source", ""))
				for split_offset in [Vector2(-16.0, 0.0), Vector2(16.0, 0.0)]:
					var split_position: Vector2 = game.clamp_point_to_room(enemy.global_position + split_offset, enemy.current_room)
					var split_enemy: Variant = spawn_wave_enemy_at(game, enemy.current_room, game.ENEMY_TYPE_SLIME, split_position, spawn_source)
					split_enemy.max_health = split_health
					split_enemy.current_health = split_health
					split_enemy.set_meta("slime_has_split", true)
					alive_enemies.append(split_enemy)
